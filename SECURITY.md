...
- Treat reference URLs as untrusted metadata. The `user-url` provider validates them, strips query strings and fragments before persistence, and never performs a server-side request, thumbnail, redirect, or image download.
- The built-in Wikimedia provider calls only the fixed Commons MediaWiki API. Import is bound to the exact result ID and metadata shown by the immediately preceding preview; the run does not repeat the search and silently substitute a different first result. The separate importer permits only the fixed `upload.wikimedia.org` / `thumb.wikimedia.org` host allowlist under `/wikipedia/commons/`, refuses redirects, limits response bytes and decoded dimensions/pixels, checks JPEG/PNG/WebP content against search metadata, and requires source, author/attribution, and an allowlisted CC/Public Domain record. It stores a metadata-stripped local PNG and provenance manifest. The renderer reads only the local file; imported pixels are never sent to the planning or image-generation model. When a real reference is used in layout, its title, creator/attribution, license, source platform, and modification notice are rendered visibly into SVG/PDF/PNG, with full URLs retained in SVG metadata and the manifest.
